...
import "../../interfaces/IJonesVault.sol";
import "openzeppelin/contracts/token/ERC20/IERC20.sol";
import "openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

/**
 * TODO: Go through the Jones Functions to see what each does.
 * Accounting for weth deposits
 */

contract JonesStratey {
    using SafeERC20 for IERC20;
    IJonesVaultV3 public jones;

    address public constant WETH = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1;
    address public constant JONES_ETH =
        0x662d0f9Ff837A51cF89A1FE7E0882a906dAC08a3;

    address public yieldDistributor;
    uint256 deposits;

    constructor(address _yieldDistributor) {
        yieldDistributor = _yieldDistributor;
    }

    function depositIntoStrategy(uint256 _amount) external {
        IERC20(WETH).safeIncreaseAllowance(address(jones), _amount);
        uint256 deposited = jones.deposit(_amount, address(this));
        deposits += deposited;
    }
    ...
    function withdraw(uint256 _amount) external {
        IERC20(JONES_ETH).safeIncreaseAllowance(address(jones), _amount);
        uint256 withdrawn = jones.withdraw(_amount, address(this), address(0));
        uint256 profit = withdrawn - deposits;
        deposits -= withdrawn;
        IERC20(WETH).safeTransfer(yieldDistributor, profit);
    }
