...
contract Communicator is NonblockingLzApp {
    ...
    constructor(address _endpoint, address _admin) NonblockingLzApp(_endpoint) {
    ...
    function setTreasury(address _treasury) public onlyOwnerOrAdmin {
        if (set != false) {
            revert TreasurySet();
        }
    ...
    function lockTreasuryAddress() external onlyOwnerOrAdmin {
        set = true;
    }
